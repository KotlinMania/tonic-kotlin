# Claude Code Project Instructions — tonic-kotlin

## Project Overview

This is **tonic-kotlin**, a clean-room Kotlin Multiplatform port of the upstream Rust crate [`tonic`](https://crates.io/crates/tonic).

The upstream Rust source is the read-only translation oracle. When porting begins, clone it into `tmp/tonic/` (gitignored). **Never edit `tmp/`.** If upstream looks wrong, the bug is in the port or in your understanding of Rust, not in `tmp/`.
...
```kotlin
// port-lint: source <path-relative-to-tmp/tonic>
